...
`main.py`, `app/main.py`, `fanpage_server.py`, and `asgi.py` now all import the same shared app from `server_core.py`, avoiding circular-import failure when one entrypoint resolves before another.
...
- source in `main.py` should be: `from server_core import app`
...
This repo centralizes the ASGI app in `server_core.py`. Even if an old `main.py` still imports `app.main`, `app.main` no longer imports `main`, so the circular import is broken.
